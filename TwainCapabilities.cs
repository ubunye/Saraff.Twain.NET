...
namespace Saraff.Twain
{
    ...
    public sealed class TwainCapabilities : MarshalByRefObject
    {
        ...
        internal TwainCapabilities(Twain32 twain)
        {
            var createCapability =
                typeof(TwainCapabilities).GetMethod("CreateCapability", BindingFlags.Instance | BindingFlags.NonPublic);
            foreach (var prop in typeof(TwainCapabilities).GetProperties())
            {
                var attrs = prop.GetCustomAttributes(typeof(CapabilityAttribute), false);
                if (attrs.Length <= 0)
                    continue;

                var attr = attrs[0] as CapabilityAttribute;

                if (attr == null)
                    continue;

                _caps.Add(attr.Cap, prop.PropertyType);
                if (createCapability != null)
                    prop.SetValue(this,
                        createCapability.MakeGenericMethod(prop.PropertyType.GetGenericArguments()[0])
                            .Invoke(this, new object[] { twain, attr.Cap }), null);
            }
        }

        private Capability<T> CreateCapability<T>(Twain32 twain, TwCap cap)
        {
            return Activator.CreateInstance(typeof(Capability<T>), twain, cap) as Capability<T>;
        }
        ...
        private class Capability<T> : ICapability<T>, ICapability2<T>
        {
            ...
            protected Twain32 Twain32 { get; }

            protected TwCap Cap { get; }

            public Twain32.Enumeration Get()
            {
                return ToEnumeration(Twain32.GetCap(Cap));
            }

            public T GetCurrent()
            {
                return (T)Twain32.GetCurrentCap(Cap);
            }

            public T GetDefault()
            {
                return (T)Twain32.GetDefaultCap(Cap);
            }

            public void Set(T value)
            {
                if (Twain32.Capabilities._caps[Cap] == typeof(ICapability2<T>) || !GetCurrent().Equals(value))
                    Twain32.SetCap(Cap, value);
            }
            ...
            {
                Twain32.SetConstraintCap(Cap, value);
            }
            ...
            {
                var val = new object[value.Length];
                for (var i = 0; i < val.Length; i++) val[i] = value[i];
                Twain32.SetConstraintCap(Cap, val);
            }
            ...
            {
                if (value.CurrentValue.GetType() != typeof(T)) throw new ArgumentException();
                Twain32.SetConstraintCap(Cap, value);
            }
            ...
            public void Reset()
            {
                Twain32.ResetCap(Cap);
            }

            public TwQC IsSupported()
            {
                return Twain32.IsCapSupported(Cap);
            }

            public bool IsSupported(TwQC operation)
            {
                return (IsSupported() & operation) == operation;
            }

            public object[] GetCurrentArray()
            {
                return ToEnumeration(Twain32.GetCurrentCap(Cap)).Items;
            }

            public object[] GetDefaultArray()
            {
                return ToEnumeration(Twain32.GetDefaultCap(Cap)).Items;
            }
            ...
            public void Set(params T[] value)
            {
                var val = new object[value.Length];
                for (var i = 0; i < val.Length; i++) val[i] = value[i];
                Twain32.SetCap(Cap, val);
            }

            private Twain32.Enumeration ToEnumeration(object value)
            {
                var val = Twain32.Enumeration.FromObject(value);
                for (var i = 0; i < val.Count; i++)
                    val[i] = typeof(T).IsEnum ? (T)val[i] : Convert.ChangeType(val[i], typeof(T));
                return val;
            }
            ...
            public override string ToString()
            {
                ...
                return
                    $"{Cap}, {(supported == 0 ? "Not Supported" : "")}{((supported & TwQC.GetCurrent) != 0 ? $"Current = {GetCurrent()}, " : "")}{((supported & TwQC.GetDefault) != 0 ? $"Default = {GetDefault()}, " : "")}{(supported != 0 ? $"Supported = {{{IsSupported()}}}" : "")}";
            }
        }
    ...
    public interface ICapability<T>
    {
        ...
        /// </summary>
        /// <returns>Capability Values.
        ///     <para xml:lang="ru">Значения возможности (capability).</para>
        /// </returns>
        ...
        /// </summary>
        /// <returns>Current Capability Value.
        ///     <para xml:lang="ru">Текущее значение возможности (capability).</para>
        /// </returns>
        T GetCurrent();
        ...
        /// </summary>
        /// <returns>Feature Default (capability).
        ///     <para xml:lang="ru">Значение по умолчанию возможности (capability).</para>
        /// </returns>
        T GetDefault();
        ...
        /// </summary>
        /// <returns>Set of flags of supported operations.
        ///     <para xml:lang="ru">Набор флагов поддерживаемых операций.</para>
        /// </returns>
        TwQC IsSupported();
        ...
        /// </summary>
        /// <param name="operation">A set of bit flags defining the required operations.
        ///     <para xml:lang="ru">Набор битовых флагов, определяющих требуемые операйии.</para>
        /// </param>
        /// <returns>True, if all specified operation is supported, otherwise false.
        ///     <para xml:lang="ru">Истина, если все указанные операции поддерживаются, иначе лож.</para>
        /// </returns>
    ...
    public interface ICapability2<T>
    {
        ...
        /// </summary>
        /// <returns>Current values of capability.
        ///     <para xml:lang="ru">Текущие значения возможности (capability).</para>
        /// </returns>
        ...
        /// </summary>
        /// <returns>The default values are capability.
        ///     <para xml:lang="ru">Значения по умолчанию возможности (capability).</para>
        /// </returns>
        ...
        ///     <para xml:lang="ru">Определяет поддерживаются ли указанные операции.</para>
        /// </summary>
        /// <param name="operation">A set of bit flags defining the required operations. The operation.
        ///     <para xml:lang="ru">Набор битовых флагов, определяющих требуемые операйии.</para>
        /// </param>
        /// <returns>True, if all specified operation is supported, otherwise false.
        ///     <para xml:lang="ru">Истина, если все указанные операции поддерживаются, иначе лож.</para>
