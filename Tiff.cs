...
namespace Saraff.Twain
{
    internal sealed class Tiff : _ImageHandler
    {
        ...
        private Ifd ImageFileDirectory
        {
            get
            {
                if (!HandlerState.ContainsKey("ImageFileDirectory"))
                    HandlerState.Add("ImageFileDirectory",
                        Ifd.FromPtr((IntPtr)(ImagePointer.ToInt64() + Header.dirOffset), ImagePointer));
        ...
        {
            if (Header.magic == MagicValues.BigEndian) throw new NotSupportedException();
        ...
        protected override int GetSize()
        {
            ...
            var size = 0;
            for (var idf = ImageFileDirectory; idf != null; idf = idf.NextIfd)
            {
                if (idf.Offset + idf.Size > size) size = idf.Offset + idf.Size;

                IfdEntry stripOffsetsTag = null, stripByteCountsTag = null;
                IfdEntry freeOffsets = null, freeByteCounts = null;
                IfdEntry tileOffsets = null, tileByteCounts = null;
                foreach (var entry in idf)
                {
                    if (entry.DataOffset + entry.DataSize > size) size = entry.DataOffset + entry.DataSize;
                    switch (entry.Tag)
                    {
                        case TiffTags.STRIPOFFSETS:
                            stripOffsetsTag = entry;
                            break;
                        case TiffTags.STRIPBYTECOUNTS:
                            stripByteCountsTag = entry;
                            break;
                        case TiffTags.FREEOFFSETS:
                            freeOffsets = entry;
                            break;
                        case TiffTags.FREEBYTECOUNTS:
                            freeByteCounts = entry;
                            break;
                        case TiffTags.TILEOFFSETS:
                            tileOffsets = entry;
                            break;
                        case TiffTags.TILEBYTECOUNTS:
                            tileByteCounts = entry;
                            break;
                    }
                }
                ...
                {
                    new[] {stripOffsetsTag, stripByteCountsTag}, new[] {freeOffsets, freeByteCounts},
                    new[] {tileOffsets, tileByteCounts}
                })
                    if (item[0] != null && item[1] != null && item[0].Length == item[1].Length)
                        for (var i = 0; i < item[0].Length; i++)
                        {
                            var dataOffset = Convert.ToInt32(item[0][i]);
                            var dataSize = Convert.ToInt32(item[1][i]);
                            if (dataOffset + dataSize > size) size = dataOffset + dataSize;
                        }
            }

            HandlerState.Add("TIFFSIZE", size);
        ...
        private sealed class TiffHeader
        {
            ...
            /// </summary>
            [MarshalAs(UnmanagedType.U2)] public MagicValues magic;
        ...
        private sealed class TiffDirEntry
        {
            ...
            ///     Tag id.
            /// </summary>
            [MarshalAs(UnmanagedType.U2)] public TiffTags tag;

            /// <summary>
            ///     Data type.
            /// </summary>
            [MarshalAs(UnmanagedType.U2)] public TiffDataType type;
        }
        ...
        private sealed class IfdEntry
        {
            ...
            {
                var entry = (TiffDirEntry)Marshal.PtrToStructure(ptr, typeof(TiffDirEntry));

                var ifdEntry = new IfdEntry
                {
                    Tag = entry.tag,
                    DataType = entry.type,
                    _data = Array.CreateInstance(TiffDataTypeHelper.Typeof(entry.type),
                        entry.count <= int.MaxValue ? entry.count : 0)
                };

                var dataPtr =
                    (IntPtr)((ifdEntry.DataSize = TiffDataTypeHelper.Sizeof(entry.type) * ifdEntry._data.Length) > 4
                        ? baseAddr.ToInt64() + entry.offset
                        : ptr.ToInt64() + 8);
                ifdEntry.DataOffset = (int)(dataPtr.ToInt64() - baseAddr.ToInt64());
                for (int i = 0, size = TiffDataTypeHelper.Sizeof(entry.type); i < ifdEntry._data.Length; i++)
                    ifdEntry._data.SetValue(
                        Marshal.PtrToStructure((IntPtr)(dataPtr.ToInt64() + size * i),
                            TiffDataTypeHelper.Typeof(entry.type)), i);
        ...
        private sealed class Ifd : Collection<IfdEntry>
        {
            ...
            {
                var idf = new Ifd();
                idf.Load(ptr, baseAddr);
                return idf;
            }

            private void Load(IntPtr ptr, IntPtr baseAddr)
            {
                var count = (ushort)Marshal.PtrToStructure(ptr, typeof(ushort));
                var intPtr = (IntPtr)(ptr.ToInt64() + sizeof(ushort));
                for (int i = 0, size = Marshal.SizeOf(typeof(TiffDirEntry));
                    i < count;
                    i++, intPtr = (IntPtr)(intPtr.ToInt64() + size)) Add(IfdEntry.FromPtr(intPtr, baseAddr));
                var nextIdfOffset = (int)Marshal.PtrToStructure(intPtr, typeof(int));
                if (nextIdfOffset != 0) NextIfd = FromPtr((IntPtr)(baseAddr.ToInt64() + nextIdfOffset), baseAddr);
                Offset = (int)(ptr.ToInt64() - baseAddr.ToInt64());
                Size = 6 + Marshal.SizeOf(typeof(TiffDirEntry)) * count;
            }
        }
        ...
        private enum TiffTags : ushort
        {
            ...
            */
            ...
            BAYERGREENSPLIT =
                50733, /* &how closely the values of the green pixels in the blue/green rows track the values of the green pixels in the red/green rows */
            ...
            MAKERNOTESAFETY =
                50741, /* &whether the EXIF MakerNote tag is safe to preserve along with the rest of the EXIF data */
